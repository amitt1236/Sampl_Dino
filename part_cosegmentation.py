...
def find_part_cosegmentation(image_paths: List[str], elbow: float = 0.975, load_size: int = 224, layer: int = 11,
                             ...
                             elbow_second_stage: float = 0.94, head: str = "student" , model_dir: str = "",save_dir: str = None) -> Tuple[List[Image.Image],
                                                                                              List[Image.Image]]:
    """
    finding cosegmentation of a set of images.
    :param model_dir: pre trained model directory
    ...
    extractor = ViTExtractor(model_type, stride, device=device, head=head, model_dir=model_dir)
    descriptors_list = []
    saliency_maps_list = []
    image_pil_list = []
    num_patches_list = []
    ...
    if low_res_saliency_maps:
        saliency_extractor = ViTExtractor(model_type, stride=8, device=device, head=head, model_dir=model_dir)
    else:
    ...
    with torch.no_grad():
        ...
        for set_dir in tqdm(sets_dir):
            ...
            parts_imgs, pil_images = find_part_cosegmentation(curr_images, args.elbow, args.load_size, args.layer,
                                                              ...
                                                              args.elbow_second_stage, save_dir=curr_save_dir)
